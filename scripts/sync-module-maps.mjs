...
const defs = [
  {folder:'product-research', page:'product-research', id:'files', title:['Product research','产品研究'], required:[shared],
   note:['One workflow file contains three modes. Read it, then apply the mode relevant to the request.','三个模式位于同一个工作流文件中：读取该文件，再执行与任务相关的模式。'],
   routes:[item('01-product-research.prompt.md','Research workflow','研究工作流','Select research planning, source research, or synthesis of supplied evidence.','选择研究规划、来源检索或已有证据归纳。')],
   refs:[general,item('../product-ux-rethinking/README.md','Product framing','产品定义','When findings are ready to inform what to build.','研究结论需要转成产品方案时读取。'),testing,analytics,context],
